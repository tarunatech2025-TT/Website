...
export default function ConsultationModal({ isOpen, onOpenChange }) {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('[ConsultationModal] Submitting payload:', {
    ...
    try {
      await emailjs.send(
        ...
        process.env.NEXT_PUBLIC_EMAILJS_TEMPLATE_ID,   // template_5ckni2m
        {
          form_type: 'Consultation Request',
          name:      form.name,
          email:     form.email,
      ...
      console.error('[ConsultationModal] EmailJS error:', err);
  ...
  // ── Buttons — shared between desktop and mobile ───────────────────────────
  const Buttons = () => (
    <div className={`flex gap-3 ${isMobile ? 'flex-col pt-1' : 'pt-2'}`}>
      ...
      <div className="flex-1 relative overflow-hidden rounded-xl">
        <motion.button type="submit"
          ...
          onMouseEnter={e => { if (!sending) e.currentTarget.style.boxShadow = '0 6px 30px rgba(168,85,247,0.65), 0 0 0 1px rgba(236,72,153,0.40)'; }}
          onMouseLeave={e => { e.currentTarget.style.boxShadow = '0 4px 22px rgba(168,85,247,0.42), 0 0 0 1px rgba(236,72,153,0.22)'; }}>
      ...
      <motion.button type="button" onClick={close}
        ...
        onMouseEnter={e => { e.currentTarget.style.background = 'rgba(168,85,247,0.10)'; e.currentTarget.style.borderColor = 'rgba(168,85,247,0.38)'; e.currentTarget.style.boxShadow = '0 0 14px rgba(168,85,247,0.18)'; }}
        onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.04)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.12)'; e.currentTarget.style.boxShadow = 'none'; }}>
        Maybe Later
      </motion.button>
    </div>
  );
